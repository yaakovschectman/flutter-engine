...
//
// ITextProvider impelementation.
//

// TODO(schectman): implement these methods
// https://github.com/flutter/flutter/issues/114547

IFACEMETHODIMP AXPlatformNodeWin::get_DocumentRange(ITextRangeProvider** result) {
  *result = GetRangeFromChild(this, this);
  return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::get_SupportedTextSelection(SupportedTextSelection* result) {
...
IFACEMETHODIMP AXPlatformNodeWin::GetSelection(SAFEARRAY** result) {
  *result = nullptr;
  ...
  // TODO(schectman): May need to implement CreateTextPositionAt to mirror
  // https://github.com/chromium/chromium/blob/main/content/browser/accessibility/browser_accessibility.cc#L725
  // within FlutterPlatformNodeDelegate
  ...
  *result = selections_to_return.Release();

  return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::GetVisibleRanges(SAFEARRAY** result) {
  const gfx::Rect frame_rect = delegate_->GetBoundsRect(AXCoordinateSystem::kFrame, AXClippingBehavior::kClipped);
  const auto start = delegate_->CreateTextPositionAt(0);
  ...
  while (!current_line_start->IsNullPosition() && *current_line_start < *end) {
    ...
    gfx::Rect current_rect = delegate_->GetInnerTextRangeBoundsRect(
  ...
  *result = scoped_visible_ranges.Release();

  return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::RangeFromChild(IRawElementProviderSimple* child, ITextRangeProvider** result) {
  *result = nullptr;
  ...
  if (!IsDescendant(child_platform_node.Get()))
    return E_INVALIDARG;

  *result = GetRangeFromChild(this, child_platform_node.Get());

  return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::RangeFromPoint(UiaPoint uia_point, ITextRangeProvider** result) {
  *result = nullptr;
  ...
  AXPlatformNodeWin* nearest_node =
      static_cast<AXPlatformNodeWin*>(NearestLeafToPoint(point));
  ...
  *result = AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(
      std::move(start), std::move(end));
  return S_OK;
}
...
ITextRangeProvider* AXPlatformNodeWin::GetRangeFromChild(
