...
void PlatformHandler::RequestAppExit(ExitType exit_type, UINT exit_code, HWND window) {
  auto callback = std::make_unique<MethodResultFunctions<rapidjson::Document>>(
      [this, exit_code, window](const rapidjson::Document* response) {
        RequestAppExitSuccess(response, exit_code, window);
      },
      nullptr, nullptr);
...
void PlatformHandler::RequestAppExitSuccess(const rapidjson::Document* result,
  ...
  if (exit_type.compare(kExitResponseExit) == 0) {
    QuitApplication(exit_code, window);
  }
}

void PlatformHandler::QuitApplication(UINT exit_code, HWND window) {
  engine_->OnQuit(exit_code, window);
}
